// Generated by CoffeeScript 1.10.0
this.mainCtrl = angular.module('mainController', []);

this.mainCtrl.controller("MainCtrl", [
  '$scope', '$interval', '$window', 'NetManager', 'Helper', 'nodeValidator', function($scope, $interval, $window, NetManager, Helper, nodeValidator) {
    var scattrs;
    $scope.$watch('book.jd_id', function() {
      var jd_id;
      jd_id = $scope.book.jd_id;
      if (jd_id && $scope.aio_form.$valid) {
        $scope.book.searching = true;
        return NetManager.get('/books/jd_get_isbn', {
          item_id: jd_id
        }).to($scope.book, 'isbn');
      }
    });
    $scope.$watch('book.isbn', function() {
      var isbn;
      $scope.error_msg = "";
      isbn = $scope.book.isbn;
      if (isbn && $scope.aio_form.$valid) {
        $scope.book.searching = true;
        return NetManager.get('/books/sniffer', {
          isbn: isbn
        }).then(function(data) {
          if (data.src !== 'none') {
            $scope.borrow_btn = true;
          }
          $scope.book.searching = false;
          $scope.book.cover = data.cover;
          $scope.book.name = data.name;
          $scope.book.author = data.author;
          $scope.book.source = data.src;
          return $scope.book.show = true;
        });
      } else {
        $scope.book.show = false;
        return $scope.borrow_btn = false;
      }
    });
    scattrs = {
      aio: {
        input: void 0
      },
      book: {
        isbn: '',
        jd_id: null
      },
      isbn_pattern: /^\d+$/,
      input_is_correct: function(allowempty) {
        var ret;
        if (allowempty === void 0) {
          allowempty = true;
        }
        ret = (allowempty && !$scope.aio_form.input.$viewValue) || $scope.aio_form.$valid;
        return ret;
      },
      author_brief: function(book) {
        return Helper.brief(book.author, 12);
      },
      handle_clear: function() {
        $scope.book = {
          isbn: '',
          jd_id: null
        };
        $('#aio-input').val('');
        $scope.aio_form.isbn.$viewValue = "";
        $scope.response_msg = "";
        return $scope.borrow_btn = false;
      },
      handle_borrow: function(event) {
        if (!$scope.aio_form.$valid) {
          return event.preventDefault();
        } else {
          return NetManager.post('/books/borrow_by_isbn', {
            book: {
              book_info_attributes: $scope.book
            }
          }).then(function(data) {
            var u;
            $scope.borrow_btn = false;
            $scope.book.borrow_state = data.status;
            if (data.status === 'ok') {
              $interval(function() {
                $scope.handle_clear();
                return $scope.borrowed_books = data.borrowed_books;
              }, 1000, 1);
              $scope.response_msg = "借书成功！";
            }
            if (data.status === 'fail') {
              if (data.errno === 'borrowed') {
                $scope.ok_btn = true;
                u = data.users[0];
                return $scope.response_msg = "该书已被" + u.nickname + "(" + u.account + ")借阅！";
              }
            }
          });
        }
      },
      handle_ret: function(event, book_id) {
        if (event.button !== 0) {
          return;
        }
        $scope.ret_book = book_id;
        return $scope.ret_timer = $interval(function() {
          return NetManager.post('/books/ret', {
            book: {
              id: book_id
            }
          }).then(function(data) {
            console.log(data);
            $scope.ret_book = null;
            return $scope.borrowed_books = data.borrowed_books;
          });
        }, 1000, 1);
      },
      handle_cancel_ret: function() {
        $scope.ret_book = null;
        return $interval.cancel($scope.ret_timer);
      }
    };
    return angular.extend($scope, scattrs);
  }
]).directive('allInOne', [
  'nodeValidator', function(validator) {
    return {
      require: 'ngModel',
      restrict: 'A',
      link: function(scope, element, attrs, controller) {
        var updateModel, updateView;
        updateView = function(value) {
          controller.$viewValue = value;
          return controller.$render();
        };
        updateModel = function(value) {
          controller.$modelValue = value;
          return $scope.ngModel = value;
        };
        return controller.$validators.allInOne = function(modelValue, viewValue) {
          var isbn_ok, item_id, jd_ok, jd_url_ok, jdv;
          if (controller.$isEmpty(modelValue)) {
            return true;
          }
          isbn_ok = validator.isISBN(viewValue);
          if (isbn_ok) {
            scope.book.isbn = viewValue;
            return true;
          }
          jd_url_ok = viewValue.match(/(http(s|):\/\/|)item.jd.com\/(\d+).html/);
          if (jd_url_ok) {
            item_id = jd_url_ok[3];
            scope.book.jd_id = item_id;
            jdv = 'JD-' + item_id;
            updateView(jdv);
            return true;
          }
          jd_ok = viewValue.match(/JD\-(\d+)/);
          if (jd_ok) {
            console.log(jd_ok);
            scope.book.jd_id = jd_ok[1];
            return true;
          }
          return false;
        };
      }
    };
  }
]);
